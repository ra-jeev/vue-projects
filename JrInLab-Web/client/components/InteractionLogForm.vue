  ...
  <v-form ref="form" lazy-validation @submit.prevent="onFormSubmit">
    <v-card outlined>
      ...
      <v-card-text>
        ...
        <v-row>
          <v-col cols="12"> Meta Details: </v-col>
          <v-col cols="12">
            <v-combobox
              ref="tags"
              v-model="form.tags"
              :append-outer-icon="editTags ? '$mdiCheck' : '$mdiPencil'"
              :items="globalConfig.tags"
              :readonly="!editTags"
              :search-input.sync="tagSearch"
              chips
              hide-selected
              hint="Maximum of 5 tags"
              label="Add searchable user tags"
              multiple
              outlined
              persistent-hint
              @click:append-outer="
          ...
          <v-col cols="12" sm="6">
            <v-select
              ...
              :items="languages"
              label="Customer's preferred language"
              chips
              outlined
              clearable
            />
          ...
          <v-col cols="12" sm="6">
            <v-select
              ...
              :items="discounts"
              label="Discounts offered (if any)"
              multiple
              chips
              outlined
            />
...
export default {
  ...
  computed: {
    ...
    discounts() {
      const discounts = this.globalConfig.discounts.filter((discount) => {
        if (this.request.discounts) {
          return !this.request.discounts.includes(discount.value)
        }
    ...
    languages() {
      const languages = this.globalConfig.languages.filter((language) => {
        if (this.request.languages) {
          return !this.request.languages.includes(language.value)
        }
  ...
  methods: {
    ...mapActions('admin', ['logCustomerInteraction']),
    ...
    async logInteraction() {
      ...
      if (this.form.language) {
        log.language = this.form.language
      }

      if (this.form.discounts) {
        log.discounts = this.form.discounts
      }
      ...
      if (this.form.tags) {
        ...
        this.form.tags.sort()
        if (this.request.tags) {
          const existingTags = [...this.request.tags].sort()
          if (this.form.tags.join() === existingTags.join()) {
            sendTags = false
          }
        }

        if (sendTags) {
          log.tags = this.form.tags
        }
      }

      try {
        await this.logCustomerInteraction({ log })
