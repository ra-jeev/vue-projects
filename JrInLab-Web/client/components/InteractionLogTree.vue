  ...
  <div>
          ...
          <v-card class="grey darken-2">
            ...
            >
              ...
              >
                ...
                <div>
                  <v-chip
                    v-for="(discount, index) in log.discounts"
                    :key="discount"
                    :class="index ? 'ml-2' : 'ml-0'"
                    label
                    outlined
                    small
                    color="white"
    ...
    <div class="hidden-sm-and-up pt-4">
      ...
      <template v-for="(log, index) in logs">
        <div :key="log.id" :class="{ 'mt-6': index }">
          <v-row align="center" no-gutters>
            <v-avatar
              :class="
                log.status ? states[log.status.new].color : 'blue darken-2'
              "
              size="36"
            >
              <v-icon>
          ...
          <v-card class="grey darken-2 mt-2">
            ...
            <v-card-text class="timeline-text">
              ...
              >
                ...
                <div>
                  <v-chip
                    v-for="(discount, ind) in log.discounts"
                    :key="discount"
                    :class="ind ? 'ml-2' : 'ml-0'"
                    label
                    outlined
...
export default {
  ...
  methods: {
    ...mapActions('admin', ['editCustomerInteraction']),
    editLog(log) {
      this.logInEdit = {}
    ...
    async submitChanges() {
      if (this.$refs.form[0].validate()) {
        ...
        if (hasChanges) {
          this.loading = true
          try {
            await this.editCustomerInteraction({ id: this.logInEdit.id, data })
            this.logInEdit = null
          } catch (error) {
